...
However, this does not fix the problem because cin FAILS to read a valid number to the integer x it ALSO sets the FAIL STATE, another hidden variable in c++ called FAIL BIT or FAIL STATE which initially is set to FALSE but as soon cin fails it sets that bit to TRUE and ANY OPERATION
